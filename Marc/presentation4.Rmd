...
SETOSA  
```{r}
reseau2 = neuralnet(Species == "setosa" ~ Sepal.Length + Sepal.Width + Petal.Length + Petal.Width, data = iris.app, linear.output = FALSE)
...
SETOSA  
```{r}
reseau2 = neuralnet(Species == "versicolor" ~ Sepal.Length + Sepal.Width + Petal.Length + Petal.Width, data = iris.app, linear.output = FALSE)
iris.app.prob.versicolor <- prediction.2.seto[,2]
iris.app.versicolor <- as.numeric(iris.app$Species == "versicolor")
seuil <- seq(0, 1, length.out = 100)
vsens <- rep(NA, 100)
vspe <- rep(NA, 100)
for (i in 1:90) {
  iris.pred <- as.numeric(iris.app.prob.versicolor >= seuil[i])
