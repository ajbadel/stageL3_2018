...
# Neural network ![Réseau de neurones](neurone.jpg)
...
## Déroulement du stage
  ...
  - Apprentissage de la méthode de prédiction choisie et de sa librairie
```{r, out.width = "340px"}
...
```
  ...
  - Application aux données "membranes placentaires"

## Objectif du stage

Problématique :  

**Établir un modèle permettant de prédire si une molécule donnée traverse la membrane placentaire.**

```{r, out.width = "340px"}
knitr::include_graphics("placenta.png")
```

  - Méthode utilisée : utilisation d'un réseau de neurone
...
Le jeu de données comporte :  
  ...
  - 90 molécules décrites par  
  - 207 descripteurs calculées avec le logiciel e-dragon  
...
boxplot(placenta1[,c(2, 75:90)], main = "Boxplot du jeu de données")
```
...
# Les réseaux de neurones et les données 'Iris de Fisher'

## Théorie

<span style='font-size:20px; border:1px'> Source : "Les réseaux de neurones peuvent s'utiliser pour extraire des modèles et détecter des tendances reposant sur des fonctions mathématiques compliquées trop difficiles, voire impossible, à modéliser à l'aide de techniques analytiques ou paramétriques traditionnelles." </span>

![Un neurone](nn1.png)
<span style='font-size:20px; border:1px'> Figure 1 : Un neurone 

![Un réseau de neurone](nn2.png)
</span> <span style='font-size:20px; border:1px'> Figure 2 : Un réseau de neurone </span>  

<span style='font-size:9px; border:1px'> 
...
<span style='font-size:25px; border:1px'>
L'échantillon d'apprentissage est constitué de 2/3 des individus du jeu de données.  
L’échantillon d’apprentissage est celui où fait notre étude que nous supposons être représentatives. 
</span>

<span style='font-size:25px; border:1px'>
L'échantillon de validation est constitué de 1/3 des individus du jeu de données.  
L’échantillon de validation permet de valider l’échantillon d’apprentissage, c’est-à-dire valider le modèle s’il est correct.
</span>
...
Val1 = c("TRUE NEGATIVE (TN)", "FALSE NEGATIVE (FN)")
Val2 = c("FALSE POSITIVE (FP)", "TRUE POSITIVE (TP)")
...
Les critères : 
  ...
  - Sensitivité = TN / (TN + FP)  
  - F1-score = 2 x (Sensibilite x Sensitivité) / (Sensibilité + Sensitivité)  
...
<span style='font-size:10px; border:1px'> http://www.adscience.fr/uploads/ckfiles/files/html_files/StatEL/statel_courbe_ROC.htm </span>
...
## Plusieurs essais réalisés 
  ...
  - 1 neurone  
  - 1 couche et n neurones  
  - 2 couches et 2n - n neurones  
  - 2 couches et n - 2n neurones  

À partir des différents essais réalisés, nous allons montrer lequel des essais est le meilleur.
...
kable(data.frame(Paramètres, Valeurs.2.app, Valeurs.2.val))
```

## Prédiction pour n nombres de neurones pour 1 couche
```{r}
...
## Prédiction pour 2n/n de neurones pour 2 couches
```{r}
...
## Prédiction pour n/2n de neurones pour 2 couches
```{r}
...
Les meilleurs modèles sont :  
  ...
  - 1 couche avec 9 neurones
  - 2 couches avec 6 neurones et 3 neurones
