...
trainset_essaie = iris[ind2 == 1, ]
head(trainset_essaie,3)
testset_essaie = iris[ind2 == 2, ]
head(testset_essaie,3)
...
```{r}
network = neuralnet(versicolor + virginica + setosa ~ Sepal.Length + Sepal.Width + Petal.Length + Petal.Width, trainset, hidden = 3, algorithm = "rprop+")
informations = network$result.matrix
informations <- as.data.frame(informations)
```

L'argument "hidden" spécifie le nombre de neurones cachés.
